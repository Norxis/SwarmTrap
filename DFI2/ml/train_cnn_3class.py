#!/usr/bin/env python3
"""3-class CNN: RECON(0) vs ATTACK(1) vs CLEAN(2).

Based on proven train_cnn.py. Handles raw array columns (pkt_size_dir etc)
by expanding them, since our prep outputs arrays not pre-expanded columns.

Usage:
    python3 train_cnn_3class.py cnn_3class.parquet --epochs 50 --batch-size 16384 --folds 5 -o models/cnn_3class_v1
"""
import argparse
import json
import os
...
# Static features — use what's available (dirty/clean lack fingerprints + src_*)
CNN_STATIC_COLS = [
...
# Array column names (raw format from AIO/PV1)
ARR_NAMES = {
    'size': ['pkt_size_dir', 'size_dir_arr'],
    'flag': ['pkt_flag', 'tcp_flags_arr'],
    'iat':  ['pkt_iat_log_ms', 'iat_log_ms_arr'],
    'rtt':  ['pkt_iat_rtt', 'iat_rtt_bin_arr'],
    'ent':  ['pkt_entropy', 'entropy_bin_arr'],
}


def find_arr_col(df_cols, candidates):
    """Find which array column name exists in the dataframe."""
    for c in candidates:
        if c in df_cols:
            return c
    return None
...
def expand_array_col_fast(df_pd, col_name, length=128):
    """Expand '[1,2,3,...]' string arrays using polars vectorized ops. Returns numpy."""
    import polars as pl
    s = pl.Series(col_name, df_pd[col_name].fillna('[]').values)
    # Strip brackets
...
class DFI_CNN(nn.Module):
    def __init__(self, num_classes=3, n_static=50):
        super().__init__()
        self.size_emb = nn.Embedding(24, 12, padding_idx=0)   # vocab 23 + pad
        ...
        # Total embedding dim per position: 12+6+6+6+4 = 34
...
def main():
    ap = argparse.ArgumentParser(description='3-class CNN: RECON/ATTACK/CLEAN')
    ap.add_argument('data', help='Parquet or CSV with array columns')
    ...
    if args.data.endswith('.parquet'):
        ...
        df = pd.read_csv(args.data, engine='pyarrow', na_values=[r'\N', 'NULL'])
    ...
    print('Expanding CNN arrays...', flush=True)
    t0 = time.time()
    cols = list(df.columns)

    # Load pre-expanded columns (prep script expands arrays → flat columns)
    # Same pattern as proven train_cnn.py
    ...
    # Offset: size_dir ranges [-11..+11], add 12 so [1..23], 0=padding
    np_size = np.where(np_size == 0, 0, np_size + 12).astype(np.int64)
    ...
    print(f'  Arrays expanded in {time.time()-t0:.1f}s', flush=True)

    # Static features — use available
    ...
    print(f'Static features: {n_static}/{len(CNN_STATIC_COLS)} available', flush=True)

    np_weights = df['label_confidence'].fillna(1.0).values.astype(np.float32)

    # Groups for GroupKFold
    rng = np.random.RandomState(42)
    groups = df['actor_id'].fillna('unknown').copy()
    ...
    model_path = os.path.join(args.output, f'cnn_3class_{ts}.pt')
    metrics_path = os.path.join(args.output, f'cnn_3class_{ts}_metrics.json')
    torch.save(best_state, model_path)
    with open(metrics_path, 'w') as f:
        json.dump({
            'model': 'cnn_3class_v1', 'timestamp': ts, 'classes': CLASS_NAMES,
            'n_samples': len(y), 'n_static': n_static, 'static_cols': avail_static,
            'epochs': args.epochs, 'best_epoch_count': best_epoch_count,
