...
Usage:
    python3 train_5class.py training_5class.csv --folds 5 --gpu -o models/5class_v1
"""
import argparse
import json
import os
...
TARGET = 'label'
NUM_CLASSES = 5
CLASS_NAMES = ['RECON', 'KNOCK', 'BRUTE', 'EXPLOIT', 'CLEAN']
...
def train(csv_path, n_splits=5, output_dir=OUTPUT_DIR, gpu=False):
    ...
    model_path = os.path.join(output_dir, f'5class_{ts}.json')
    metrics_path = os.path.join(output_dir, f'5class_{ts}_metrics.json')
