#!/usr/bin/env python3
"""Prep 3-class CNN dataset: TCP-only, ingress-only. FAST polars version.

Tested on small batch first. Key fix: cast ALL feature cols to Float64 before concat.

Usage:
    python3 prep_cnn_3class_fast.py /nvme0n1-disk/ml/data -o /nvme0n1-disk/ml/data/cnn_3class.parquet
"""
import argparse
import os
import time
...
# Columns to keep as-is (strings or CNN arrays)
...
SOURCES = [
    ...
    ('clean_vlan100.csv', 2, 'clean ingress'),
]
...
def load_and_normalize(filepath, label):
    ...
    # Cast ALL non-string cols to Float64 (fixes Int64/Float64/String mismatches)
    for c in df.columns:
        if c in KEEP_STRING:
            continue
...
def main():
    ...
    ap.add_argument('-o', '--output', default=os.path.join(DATA_DIR, 'cnn_3class.parquet'))
    args = ap.parse_args()
    data = args.data_dir
    t0 = time.time()
