-- dfi_clean: Clean (non-attack) traffic captured from SPAN
-- Watchlist-filtered: any flow involving a watchlist IP is dropped
-- Only flows table + buffer needed — no labels, predictions, or scoring

CREATE DATABASE IF NOT EXISTS dfi_clean;

CREATE TABLE IF NOT EXISTS dfi_clean.flows
(
    flow_id         String,
    session_key     String,
    actor_id        String,
    ...
    -- CNN packet sequences (up to 128 tokens per flow)
...
CREATE TABLE IF NOT EXISTS dfi_clean.flows_buffer AS dfi_clean.flows
ENGINE = Buffer(dfi_clean, flows,
